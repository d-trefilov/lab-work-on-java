package lr11;

import java.util.Arrays;
import java.util.Random;
import java.util.Scanner;

public class Example1 {

    public static void main(String[] args) {
        Scanner in = new Scanner(System.in);
        System.out.println("Введите размер массива");
        int size = in.nextInt();

        int[] arr = new int[size];

        Random random = new Random();

        for (int i = 0; i < size; i++) {
            arr[i] = random.nextInt(100); // рекомендую добавить bound, чтобы числа были в разумных пределах
        }

        System.out.println("Массив arr : ");
        System.out.println(Arrays.toString(arr));

        int[] arrResult = filterEvenNumbers(arr);

        System.out.println("Массив arrResult : ");
        System.out.println(Arrays.toString(arrResult));

        in.close(); // рекомендую закрывать Scanner
    }

    /**
     * Фильтрует массив, оставляя только чётные числа
     * @param arr исходный массив
     * @return массив из чётных чисел
     */
    public static int[] filterEvenNumbers(int[] arr) {
        return Arrays.stream(arr).filter(x -> x % 2 == 0).toArray();
    }
}